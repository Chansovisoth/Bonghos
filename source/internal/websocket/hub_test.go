...
func TestMinimumIntervalUsesFastestSubscriber(t *testing.T) {
	...
	h.clients[testClient("overview_performance", 4*time.Second)] = true

	if got := h.MinimumInterval("performance", 10*time.Second); got != time.Second {
		t.Fatalf("MinimumInterval() = %v, want 1s", got)
	}
	if got := h.MinimumInterval("overview_performance", 10*time.Second); got != 4*time.Second {
		t.Fatalf("MinimumInterval(overview_performance) = %v, want 4s", got)
	}
}
